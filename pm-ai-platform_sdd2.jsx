...
/* ═══════════════════════════ DESIGN STUDIO ══════════════════════════════════ */
function DesignStudio({ cards, focusCardId, onBack, onUpdateDocs, onUpdateCard, themeMode, onToggleThemeMode }) {
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.paper}}>
      ...
      <div style={{background:theme.topBg,borderBottom:`1px solid ${theme.topBorder}`,padding:"0 0 0 0",display:"flex",alignItems:"center",gap:0,flexShrink:0,height:44}}>
        ...
        <div style={{padding:"0 18px",display:"flex",alignItems:"center",gap:8}}>
          ...
          {selCard && (
            selCard.feishuUrl ? (
              <a href={selCard.feishuUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:theme.topText,fontWeight:500,textDecoration:"none",borderBottom:`1px dashed ${theme.topText}`,cursor:"pointer"}}
                onMouseEnter={e=>{e.currentTarget.style.color=C.accent;e.currentTarget.style.borderBottomColor=C.accent;}}
                onMouseLeave={e=>{e.currentTarget.style.color=theme.topText;e.currentTarget.style.borderBottomColor=theme.topText;}}>
                {selCard.id}
              </a>
            ) : (
              <span style={{fontSize:12,color:theme.topText,fontWeight:500}}>{selCard.id}</span>
            )
          )}
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        ...
        {/* ── Right content panel ── */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:theme.rightBg,minWidth:400}}>
            ...
            {!selCard ? (
              ...
              <div style={{padding:"24px 36px",height:"100%",boxSizing:"border-box"}}>
                <textarea
                  ...
                  style={{width:"100%",height:"calc(100vh - 180px)",padding:"16px",border:`1px solid ${C.border}`,borderRadius:8,fontSize:13,color:C.ink,background:theme.editorBg,outline:"none",resize:"none",lineHeight:1.75,fontFamily:"'DM Mono',monospace",boxSizing:"borderBox"}}
